...
function StatusBadge({ status }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2.5 py-0.5 text-xs font-bold rounded-full border ${s.cls}`}
    >
      {s.icon}
      {status}
    </span>
...
function PassFailBar({ data, resultKey }) {
  ...
  const pass = data.filter((d) =>
    ["PASS", "PASSED", "OK"].includes(
      (d[resultKey] || "").toString().toUpperCase(),
    ),
  ).length;
    ...
    <div className="flex items-center gap-4 px-4 py-2.5 bg-slate-50 border-b border-slate-100">
      <div className="flex items-center gap-3 text-xs font-semibold">
        ...
        <span className="text-emerald-600 flex items-center gap-1">
          <CheckCircle className="w-3 h-3" /> {pass} Pass
        </span>
        {fail > 0 && (
          <span className="text-red-600 flex items-center gap-1">
            <XCircle className="w-3 h-3" /> {fail} Fail
          </span>
        )}
      </div>
      ...
      <span
        className={`text-xs font-black ${fail > 0 ? "text-red-500" : "text-emerald-600"}`}
      >
        {pct}% Pass
      </span>
    </div>
...
const Th = ({ children, ...props }) => (
  <th
    className="px-3 py-2.5 text-[10px] font-bold uppercase tracking-wider text-center border-r border-white/10 last:border-0 whitespace-nowrap"
    {...props}
  >
    {children}
  </th>
...
const Td = ({ children, className = "", ...props }) => (
  <td className={`px-3 py-2.5 text-xs ${className}`} {...props}>
    {children}
  </td>
);
      ...
      <div className="overflow-x-auto overflow-y-auto max-h-[420px]">
        ...
        >
          <thead className="sticky top-0 z-10">
            <tr
              className={`bg-gradient-to-r ${tabConfig.headerGrad} text-white`}
            >
              {headers.map((h, i) => (
                <Th key={i}>{h}</Th>
              ))}
...
function ESTTable({ data = [], tabConfig }) {
  const groups = [
    ...
    {
      label: "HV",
      children: ["Set kV", "Set mA", "Set T", "Read kV", "Result"],
    },
    { label: "IR", children: ["Set MΩ", "Set T", "Read MΩ", "Result"] },
    {
      label: "LCT LN",
      children: ["Set mA", "Set T", "Read mA", "Read V", "Result"],
    },
      ...
      <div className="overflow-x-auto overflow-y-auto max-h-[420px]">
        ...
        >
          <thead className="sticky top-0 z-10">
            ...
            <tr
              className={`bg-gradient-to-r ${tabConfig?.headerGrad} text-white/90`}
            >
              {groups
                .filter((g) => !single(g))
                .flatMap((g, gi) =>
          ...
          <tbody>
            {data.length > 0 ? (
              data.map((item, i) => {
                const fail =
                  item.result && item.result.toUpperCase() !== "PASS";
                const bg = fail
                  ? "bg-red-50/60"
                  : i % 2 === 0
                    ? "bg-white"
                ...
                return (
                  ...
                  >
                    {[
                      <Td className="font-bold text-slate-400 border border-slate-100">
                        {i + 1}
                      </Td>,
                      <Td className="font-mono border border-slate-100">
                        {item.RefNo ?? "—"}
                      </Td>,
                      <Td className="text-left border border-slate-100">
                        {item.model_no ?? "—"}
                      </Td>,
                      <Td className="font-mono font-semibold text-amber-700 border border-slate-100">
                        {item.serial_no ?? "—"}
                      </Td>,
              ...
              data.map((item, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <Td>
                      <span
                        className={`px-2 py-0.5 rounded-full text-[10px] font-bold border ${item.SYNCSTATUS === 1 ? "bg-emerald-50 text-emerald-700 border-emerald-200" : "bg-amber-50 text-amber-700 border-amber-200"}`}
                      >
            ...
            {data.length > 0 ? (
              data.map((item, i) => {
                ...
                return (
                  <tr
                    key={i}
                    className={`text-center transition-colors ${bg} hover:bg-rose-50/30`}
                  >
                    <Td className="font-bold text-slate-400 border border-slate-100">
                      {i + 1}
                    </Td>
                    ...
                    <Td className="font-mono font-semibold text-rose-700 border border-slate-100">
                      {item.BARCODE ?? "—"}
                    </Td>
                    <Td className="border border-slate-100">
                      {item.MODEL ?? "—"}
                    ...
                    <Td className="font-mono font-bold border border-slate-100">
                      {item.RUNTIME_MINUTES ?? "—"}
                    </Td>
                    <Td className="font-mono border border-slate-100">
                      {fmtNum(item.MIN_TEMPERATURE)}
                    </Td>
                    <Td className="font-mono border border-slate-100">
                      {fmtNum(item.MAX_TEMPERATURE)}
                    </Td>
                    <Td className="font-mono border border-slate-100">
                      {fmtNum(item.MIN_CURRENT)}
                    </Td>
                    <Td className="font-mono border border-slate-100">
                      {fmtNum(item.MAX_CURRENT)}
                    </Td>
                    <Td className="font-mono border border-slate-100">
                      {fmtNum(item.MIN_VOLTAGE)}
                    </Td>
                    <Td className="font-mono border border-slate-100">
                      {fmtNum(item.MAX_VOLTAGE)}
                    </Td>
                    <Td className="font-mono border border-slate-100">
                      {fmtNum(item.MIN_POWER)}
                    </Td>
                    <Td className="font-mono border border-slate-100">
                      {fmtNum(item.MAX_POWER)}
                    </Td>
                    <Td className="border border-slate-100">
                      <StatusBadge status={r} />
                    </Td>
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {INNER_TABS.map((tab) => {
          ...
          const pass =
            dataMap[tab.key]?.filter((d) =>
              ["PASS", "PASSED", "OK"].includes(
                (d[tab.resultKey] || "").toString().toUpperCase(),
              ),
            ).length || 0;
          ...
          return (
            ...
            >
              <div
                className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${isAct ? tab.activeBg : "bg-slate-100"}`}
              >
                <BarChart2
                  className={`w-4 h-4 ${isAct ? tab.iconActive : "text-slate-400"}`}
                />
              </div>
              <div className="min-w-0">
                ...
                {count > 0 && (
                  <p
                    className={`text-[10px] font-semibold ${hasFail ? "text-red-500" : "text-emerald-600"}`}
                  >
        ...
        <div className="flex items-center bg-slate-50/80 border-b border-slate-200 px-2 pt-1.5 gap-0.5">
          {INNER_TABS.map((tab) => {
            ...
            return (
              ...
              >
                <span className={isAct ? tab.iconActive : "text-slate-400"}>
                  <BarChart2 className="w-3.5 h-3.5" />
                </span>
                <span className="hidden md:inline">{tab.label}</span>
                <span className="md:hidden">{tab.short}</span>
                <span
                  className={`inline-flex items-center justify-center min-w-[20px] h-4 px-1 rounded-full text-[10px] font-bold ml-1 ${isAct ? tab.badge : "bg-slate-200 text-slate-500"}`}
                >
                  {count}
                </span>
              </button>
