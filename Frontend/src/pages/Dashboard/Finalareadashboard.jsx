...
import {
  FaSync, FaCalendarAlt, FaIndustry, FaChartBar,
  FaChartLine, FaExclamationTriangle, FaCheckCircle,
  FaTimesCircle, FaArrowUp, FaArrowDown, FaMinus,
  FaClipboardList, FaBolt, FaStar, FaBox, FaTruck,
  FaTachometerAlt, FaStopwatch, FaLayerGroup,
} from "react-icons/fa";
import {
  MdOutlineQueryStats, MdSpeed, MdDashboard,
  MdWarningAmber, MdCheckCircleOutline,
} from "react-icons/md";
import {
  Chart as ChartJS,
...
const GaugeCanvas = ({ value = 0, label = "", sublabel = "" }) => {
  ...
  useEffect(() => {
    ...
    // Center labels
    ctx.font = "bold 10px monospace"; ctx.fillStyle = "#d1d5db";
    ctx.fillText(label, cx, cy - 44);
    ctx.font = "9px monospace"; ctx.fillStyle = "#9ca3af";
...
const DonutCanvas = ({ pct = 0, size = 150, fillColor = "#10b981", trackColor = "#374151" }) => {
  ...
  useEffect(() => {
    ...
    ctx.lineWidth = 15; ctx.strokeStyle = trackColor;
    ctx.stroke();
    ...
    if (pct > 0) {
      ...
      ctx.lineWidth = 15; ctx.strokeStyle = fillColor; ctx.lineCap = "butt";
      ctx.stroke();
    }
...
const PageHeader = ({ title, shift, shiftDate, whiteStyle = false }) => {
  ...
  if (whiteStyle) {
    return (
      <>
        <div style={{ background: "#fff", display: "flex", alignItems: "center", gap: 12, padding: "8px 16px", borderBottom: "3px solid #1a5a9a", flexShrink: 0 }}>
          <LogoCircle />
          <div style={{ flex: 1, textAlign: "center", fontWeight: 800, fontSize: 20, color: "#111827", letterSpacing: 1, textTransform: "uppercase" }}>{title}</div>
        </div>
  ...
  return (
    <>
      <div style={{ background: "#1f2937", display: "flex", alignItems: "center", gap: 12, padding: "8px 16px", borderBottom: "1px solid #374151", flexShrink: 0 }}>
        <LogoCircle />
        <div style={{ flex: 1, textAlign: "center", fontWeight: 800, fontSize: 18, color: "#fff", letterSpacing: 1, textTransform: "uppercase" }}>{title}</div>
      </div>
...
const MetaRow = ({ shift, shiftDate, timeStr, light = false }) => {
  ...
  return (
    <div style={rowStyle}>
      <span style={lblStyle}>Shift: <span style={valStyle}>{shift ? `SHIFT ${shift}` : "—"}</span></span>
      <span style={lblStyle}>Date: <span style={valStyle}>{shiftDate || "—"}</span></span>
      <span style={lblStyle}>Time: <span style={valStyle}>{timeStr}</span></span>
...
const MetricRow = ({ label, unit, target, actual, highlight, odd }) => (
  <tr style={{ background: odd ? "#172030" : "#1f2937" }}>
    ...
    <td style={{
      padding: "6px 10px", border: "1px solid #374151", fontSize: 12, textAlign: "center", fontWeight: 700,
...
const LeftSidebar = ({ consumedPct = 0, fillColor = "#10b981", infoRows = [] }) => (
  <div style={{
    ...
    padding: 12, background: "#1a2740", borderRight: "1px solid #374151",
  ...
  <div style={{
    ...
    {PAGES_META.map((pg, i) => (
      <button key={i} onClick={() => onGoTo(i)} style={{ background: "none", border: "none", cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 3 }}>
        <div style={{
          width: i === currentPage ? 28 : 10, height: 10, borderRadius: 5,
...
// ─── Page Shared Styles ─────────────────────────────────────────────────────────
const METRIC_TH = { padding: "7px 10px", background: "#374151", color: "#d1d5db", fontSize: 11, fontWeight: 700, border: "1px solid #4b5563", textAlign: "center" };
    ...
    <div style={{ display: "flex", flexDirection: "column", width: "100%", height: "100%", background: "#111827" }}>
      ...
      <div style={{ display: "flex", flex: 1, minHeight: 0 }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", padding: 14, minWidth: 0 }}>
          <div style={{ background: "#b8cfe8", color: "#111827", fontWeight: 700, fontSize: 14, textAlign: "center", padding: "6px 0", marginBottom: 0 }}>FG PACKING</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 4, padding: "6px 8px", background: "#1f2937", borderTop: "1px solid #374151", flexShrink: 0 }}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", width: "100%", height: "100%", background: "#111827" }}>
      ...
      <div style={{ display: "flex", flex: 1, minHeight: 0 }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", padding: 14, minWidth: 0 }}>
          <div style={{ background: "#b8cfe8", color: "#111827", fontWeight: 700, fontSize: 14, textAlign: "center", padding: "6px 0" }}>FG LOADING</div>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr>
                <th style={{ ...METRIC_TH, textAlign: "left", width: "45%" }}>Metric</th>
...
const Page3 = ({ apiData = {}, progress, shift, shiftDate }) => {
  ...
  const chartData = useMemo(() => ({
    ...
    datasets: [
      { type: "bar",  label: "Target", data: hours.map(h => h.Target),  backgroundColor: "rgba(68,114,196,0.8)", borderColor: "#4472c4", borderWidth: 1, borderRadius: 4, yAxisID: "y" },
      { type: "bar",  label: "Actual", data: hours.map(h => h.Actual),  backgroundColor: "rgba(237,125,49,0.8)",  borderColor: "#ed7d31", borderWidth: 1, borderRadius: 4, yAxisID: "y" },
      { type: "line", label: "Loss",   data: hours.map(h => h.HourLoss), borderColor: "#9ca3af", borderWidth: 2,   pointRadius: 4, tension: 0.3, fill: false, yAxisID: "y2" },
    ],
  }), [hours]);
  ...
  const chartOptions = useMemo(() => ({
    ...
    scales: {
      ...
      y2: { position: "right", beginAtZero: true, ticks: { color: "#9ca3af", font: { size: 9 } }, grid: { display: false } },
    },
  }), []);
            ...
            <table style={{ minWidth: "100%", borderCollapse: "collapse", fontSize: 11 }}>
              ...
              <tbody>
                <tr>
                  ...
                  {hours.map((h, i) => (
                    <td key={i} style={{ background: "#1a3050", color: "#f1f5f9", padding: "5px 6px", border: "1px solid #2a4a70", textAlign: "center" }}>{h.CumulativeLoss}</td>
                  ))}
                </tr>
                <tr>
        ...
        <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "space-around", padding: "12px 20px", gap: 20, background: "#1e2d45", minWidth: 0 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
            ...
            <div style={{ position: "relative" }}>
              <Chart type="doughnut" data={pieChartData} options={pieOptions} width={220} height={220} />
            ...
            <div style={{ display: "flex", gap: 12, fontSize: 11 }}>
              <span style={{ display: "flex", alignItems: "center", gap: 4, color: "#86efac" }}>
                <span style={{ width: 10, height: 10, background: "#16a34a", borderRadius: 2, display: "inline-block" }} />OK: {qs.OkUnit ?? 0}
              </span>
              <span style={{ display: "flex", alignItems: "center", gap: 4, color: "#fca5a5" }}>
                <span style={{ width: 10, height: 10, background: "#dc2626", borderRadius: 2, display: "inline-block" }} />Defect: {qs.DefectUnit ?? 0}
              </span>
            </div>
          </div>
          {/* Defect Table */}
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
              ...
              <tbody>
                {defects.length > 0 ? defects.map((df, i) => (
                  ...
                  <tr>
                    <td colSpan={3} style={{ background: "#1a3050", color: "#9ca3af", padding: "20px", textAlign: "center", border: "1px solid #2a5a8a" }}>No defects recorded</td>
                  </tr>
                )}
              </tbody>
            </table>
...
const Page5 = ({ apiData = {}, progress, shift, shiftDate }) => {
  ...
  const maxTime = useMemo(() => Math.max(...stations.map(s => s.TotalStopTime), 1), [stations]);
      ...
      <div style={{ display: "flex", flex: 1, minHeight: 0 }}>
        ...
        <div style={{ flex: 1, padding: 12, overflow: "auto", background: "#1e2d45" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
            <thead>
              <tr>
                {["Station Name", "Stop Time (Min)", "Stop Count"].map((h, i) => (
              ...
              {stations.map((s, i) => {
                ...
                return (
                  <tr key={i}>
                    ...
                    <td style={{ padding: "7px 12px", border: "1px solid #4b5563", background: rowBg, color: s.TotalStopTime > 50 ? "#fca5a5" : "#d1d5db", textAlign: "center", fontWeight: 700 }}>{s.TotalStopTime}</td>
                    <td style={{ padding: "7px 12px", border: "1px solid #4b5563", background: rowBg, color: s.TotalStopCount > 10 ? "#fca5a5" : "#d1d5db", textAlign: "center", fontWeight: 700 }}>{s.TotalStopCount}</td>
                  </tr>
                );
              })}
              {stations.length === 0 && (
                <tr><td colSpan={3} style={{ padding: 20, textAlign: "center", color: "#6b7280", background: "#1f2937", border: "1px solid #374151" }}>No loss data recorded</td></tr>
              )}
            </tbody>
          </table>
        </div>
...
const FinalAreaDashboard = () => {
  ...
  const fetchAllData = useCallback(async () => {
    if (!shiftDate) { toast.error("Please select a shift date."); return; }
    setLoading(true);
    setIsRunning(false);
  ...
  const handleTodayShiftA = useCallback(() => {
    setShiftDate(todayISO()); setShift("A");
  ...
  const handleTodayShiftB = useCallback(() => {
    ...
    setShiftDate(`${d.getFullYear()}-${pad(d.getMonth()+1)}-${pad(d.getDate())}`);
    setShift("B");
    setTimeout(fetchAllData, 0);
  }, [fetchAllData]);
    ...
    <div style={{ padding: 20, background: "#f1f5f9", minHeight: "100vh", fontFamily: "system-ui, -apple-system, sans-serif" }}>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          <div style={{ background: "rgba(255,255,255,0.1)", borderRadius: 10, padding: 10, fontSize: 24, color: "#fff" }}>
            <MdDashboard />
          </div>
          <div>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 20, alignItems: "flex-end" }}>
          ...
          <div>
            ...
            <input
              ...
              style={{ padding: "7px 12px", border: "1px solid #d1d5db", borderRadius: 7, fontSize: 13, outline: "none" }}
            />
          </div>
          ...
          <div>
            ...
            <div style={{ display: "flex", background: "#f9fafb", border: "1px solid #e5e7eb", borderRadius: 8, overflow: "hidden" }}>
              {SHIFT_OPTIONS.map((opt) => (
                <button key={opt.value} onClick={() => setShift(opt.value)} style={{
                ...
                }}>{opt.value === "A" ? "Shift A" : "Shift B"}</button>
              ))}
            </div>
          </div>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
            ...
            <button onClick={handleTodayShiftA} disabled={loading} style={{
            ...
            }}><FaStar size={10} /> TODAY A</button>

            <button onClick={handleTodayShiftB} disabled={loading} style={{
              display: "flex", alignItems: "center", gap: 5, padding: "9px 14px",
              background: "#fef3c7", color: "#92400e", border: "1px solid #fde68a",
              borderRadius: 8, fontWeight: 700, fontSize: 12, cursor: loading ? "not-allowed" : "pointer",
            }}><FaBolt size={10} /> TODAY B</button>
...
export default FinalAreaDashboard;