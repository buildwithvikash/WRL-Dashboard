...
import {
  FiRefreshCw, FiPlay, FiPause, FiX, FiClock, FiCalendar,
  FiAlertTriangle, FiCheckCircle, FiPackage,
  FiTruck, FiBarChart2, FiActivity, FiSettings, FiTrendingUp,
...
const PAGES_META = [
  { key: "fgPacking", label: "FG Packing",  Icon: FiPackage,       accent: "#1e40af", light: "#dbeafe" },
  { key: "fgLoading", label: "FG Loading",  Icon: FiTruck,         accent: "#0f766e", light: "#ccfbf1" },
  { key: "hourly",    label: "Hourly",       Icon: FiBarChart2,     accent: "#7c3aed", light: "#ede9fe" },
  { key: "quality",   label: "Quality",      Icon: FiCheckCircle,   accent: "#15803d", light: "#dcfce7" },
  { key: "loss",      label: "Loss",         Icon: FiAlertTriangle, accent: "#b45309", light: "#fef3c7" },
];
...
const GaugeCanvas = ({ value = 0, label = "", sublabel = "" }) => {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i <= 10; i++) {
      ...
      if (i % 2 === 0) {
        ctx.fillStyle = "#64748b"; ctx.font = "bold 9px 'Courier New', monospace";
        ctx.fillText(String(i * 100), cx + cos * (R - 34), cy + sin * (R - 34));
      }
    }

    ctx.font = "bold 11px 'Courier New', monospace"; ctx.fillStyle = "#1e293b";
  ...
  return <canvas ref={ref} width={280} height={160} style={{ display: "block", maxWidth: "100%" }}/>;
};

// ─── Donut Canvas ─────────────────────────────────────────────────────────────
const DonutCanvas = ({ pct = 0, size = 120, fillColor = "#22c55e", trackColor = "#e2e8f0" }) => {
  const ref = useRef(null);
  useEffect(() => {
    const c = ref.current; if (!c) return;
    const ctx = c.getContext("2d");
    ...
    ctx.lineWidth = 12; ctx.strokeStyle = trackColor; ctx.stroke();
    if (pct > 0) {
      ctx.beginPath();
      ctx.arc(cx, cy, r, -Math.PI / 2, -Math.PI / 2 + (Math.min(pct, 100) / 100) * Math.PI * 2);
      ctx.lineWidth = 12; ctx.strokeStyle = fillColor; ctx.lineCap = "round"; ctx.stroke();
    }
...
const LiveClock = ({ shift, shiftDate, accent }) => {
  ...
  return (
    <div style={{
      ...
      background: "#f8fafc", borderBottom: "1px solid #f1f5f9", fontSize: 12, flexShrink: 0,
    }}>
      <span style={{ display: "flex", alignItems: "center", gap: 5, color: "#64748b" }}>
        <FiActivity size={11} color={accent}/>
        Shift <strong style={{ color: "#0f172a", marginLeft: 2 }}>{shift ? `SHIFT ${shift}` : "—"}</strong>
      </span>
      <span style={{ display: "flex", alignItems: "center", gap: 5, color: "#64748b" }}>
      ...
      <span style={{ display: "flex", alignItems: "center", gap: 5, color: "#64748b" }}>
        ...
        <strong style={{ color: "#0f172a", fontFamily: "monospace" }}>{timeStr}</strong>
      </span>
...
const PageHeader = ({ title, shift, shiftDate, accent }) => (
  <div style={{ flexShrink: 0 }}>
    <div style={{ background: accent, display: "flex", alignItems: "center", gap: 12, padding: "9px 16px" }}>
      <div style={{
      ...
      <span style={{
        flex: 1, textAlign: "center", fontWeight: 800, fontSize: 15, color: "#fff",
...
const TimerBar = ({ progress, accent }) => (
  <div style={{ height: 3, background: "#f1f5f9", flexShrink: 0 }}>
    <div style={{ height: "100%", width: `${progress}%`, background: accent, transition: "none" }}/>
  </div>
);
...
const StatCard = ({ label, value, accent = "#1e40af" }) => (
  <div style={{
    ...
    borderTop: `3px solid ${accent}`, border: "1px solid #f1f5f9",
  }}>
    <div style={{ fontSize: 10, color: "#94a3b8", lineHeight: 1.4, marginBottom: 3 }}>{label}</div>
...
const MetricTable = ({ rows, accent }) => (
  <table style={{ width: "100%", borderCollapse: "collapse" }}>
    <thead>
      <tr>
        {[["45%","Metric","left"],["10%","Unit","center"],["22%","Target","center"],["23%","Actual","center"]].map(([w,lbl,align]) => (
          <th key={lbl} style={{
            padding: "8px 10px", background: accent, color: "#fff",
    ...
    <tbody>
      {rows.map((r, i) => {
        const actualColor = r.highlight === "yellow" ? "#f59e0b" : r.green ? "#15803d" : "#0f172a";
        return (
          <tr key={i} style={{ background: i % 2 === 0 ? "#fff" : "#f8fafc" }}>
            <td style={{ padding: "7px 10px", border: "1px solid #f1f5f9", color: "#334155", fontWeight: 600, fontSize: 12 }}>{r.label}</td>
            <td style={{ padding: "7px 10px", border: "1px solid #f1f5f9", color: "#94a3b8", fontSize: 11, textAlign: "center" }}>{r.unit}</td>
            <td style={{ padding: "7px 10px", border: "1px solid #f1f5f9", color: accent, fontWeight: 700, fontSize: 12, textAlign: "center" }}>{r.target ?? "—"}</td>
            <td style={{ padding: "7px 10px", border: "1px solid #f1f5f9", fontSize: 12, textAlign: "center", fontWeight: 800, color: actualColor }}>{r.actual ?? "—"}</td>
          </tr>
        );
      })}
    </tbody>
...
const GaugePanel = ({ value, label, sublabel, accent }) => (
  <div style={{
    ...
    background: "#fff", borderRight: "1px solid #f1f5f9", padding: "16px 12px",
  }}>
    <GaugeCanvas value={value} label={label} sublabel={sublabel}/>
    <div style={{
      marginTop: 12, padding: "6px 28px", background: accent, borderRadius: 8,
      color: "#fff", fontWeight: 800, fontSize: 24, fontFamily: "'Courier New', monospace",
      letterSpacing: 4, boxShadow: `0 4px 14px ${accent}44`,
    }}>
...
const SidebarPanel = ({ pct = 0, fillColor, infoRows = [], label = "Consumed Time" }) => (
  <div style={{
    width: 190, flexShrink: 0, display: "flex", flexDirection: "column",
    gap: 12, padding: "14px 12px", background: "#f8fafc",
    borderRight: "1px solid #f1f5f9", justifyContent: "center",
  }}>
    <div>
      <div style={{
    ...
    <div style={{ background: "#fff", border: "1px solid #f1f5f9", borderRadius: 10, padding: "10px 12px" }}>
      {infoRows.map(([k, v], i) => (
        <div key={i} style={{
          display: "flex", justifyContent: "space-between", padding: "4px 0",
          borderBottom: i < infoRows.length - 1 ? "1px solid #f8fafc" : "none",
...
const NavDots = ({ currentPage, onGoTo, onPrev, onNext }) => (
  <div style={{
    ...
    gap: 6, padding: "8px 16px", background: "#fff",
    borderTop: "1px solid #f1f5f9", position: "relative",
  }}>
    ...
    {PAGES_META.map(({ label, Icon, accent, light }, i) => (
      <button key={i} onClick={() => onGoTo(i)} style={{
        ...
        <div style={{
          ...
          border: i === currentPage ? `1.5px solid ${accent}` : "1.5px solid transparent",
...
const Page1 = ({ apiData = {}, progress, shift, shiftDate, config }) => {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", width: "100%", height: "100%", background: "#f8fafc" }}>
      <PageHeader title="Final Area Production Performance" shift={shift} shiftDate={shiftDate} accent={accent}/>
      <TimerBar progress={progress} accent={accent}/>
      <div style={{ display: "flex", flex: 1, minHeight: 0 }}>
        <GaugePanel value={d.GaugeValue ?? 0} label={label} sublabel="Units / Shift" accent={accent}/>
        <div style={{ flex: 1, display: "flex", flexDirection: "column", padding: 12, minWidth: 0, gap: 8 }}>
          <div style={{
            background: accent, color: "#fff", fontWeight: 700, fontSize: 13,
            textAlign: "center", padding: "7px 0", borderRadius: "8px 8px 0 0",
          }}>{label}</div>
      ...
      <div style={{
        ...
        <StatCard label="Monthly Plan"        value={d.MonthlyPlanQty}   accent="#1e40af"/>
        <StatCard label="Monthly Achievement" value={d.MonthlyAchieved}  accent="#15803d"/>
        <StatCard label="Remaining Qty"       value={d.MonthlyRemaining} accent="#b45309"/>
        <StatCard label="Asking Rate"         value={d.AskingRate}       accent="#0f766e"/>
        <StatCard label="Remaining Days"      value={d.RemainingDays}    accent="#64748b"/>
      </div>
    </div>
  );
};
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", width: "100%", height: "100%", background: "#f8fafc" }}>
      ...
      <div style={{
        ...
        <StatCard label="Monthly Plan"        value={d.MonthlyPlanQty}   accent={accent}   />
        <StatCard label="Monthly Achievement" value={d.MonthlyAchieved}  accent="#15803d"  />
        <StatCard label="Remaining Qty"       value={d.MonthlyRemaining} accent="#b45309"  />
        <StatCard label="Asking Rate"         value={d.AskingRate}       accent="#0f766e"  />
        <StatCard label="Remaining Days"      value={d.RemainingDays}    accent="#64748b"  />
...
const Page3 = ({ apiData = {}, progress, shift, shiftDate }) => {
  ...
  const chartData = useMemo(() => ({
    ...
    datasets: [
      { type: "bar",  label: "Target", data: hours.map(h => h.Target),   backgroundColor: "rgba(30,64,175,0.7)", borderColor: "#1e40af", borderWidth: 1, borderRadius: 4, yAxisID: "y" },
      { type: "bar",  label: "Actual", data: hours.map(h => h.Actual),   backgroundColor: "rgba(245,158,11,0.85)", borderColor: "#d97706", borderWidth: 1, borderRadius: 4, yAxisID: "y" },
      { type: "line", label: "Loss",   data: hours.map(h => h.HourLoss), borderColor: "#ef4444", borderWidth: 2, pointRadius: 4, tension: 0.3, fill: false, yAxisID: "y2" },
    ],
  }), [hours]);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", width: "100%", height: "100%", background: "#f8fafc" }}>
      <PageHeader title="Hourly Production Performance" shift={shift} shiftDate={shiftDate} accent={accent}/>
      <TimerBar progress={progress} accent="#8b5cf6"/>
      <div style={{ display: "flex", flex: 1, minHeight: 0 }}>
        <SidebarPanel
          pct={summary.ConsumedTimePct} fillColor="#8b5cf6"
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", padding: "10px 12px", gap: 8, minWidth: 0 }}>
          <div style={{ overflowX: "auto" }}>
            <table style={{ minWidth: "100%", borderCollapse: "collapse", fontSize: 11 }}>
              <thead>
                <tr>
                  <th style={{ background: accent, color: "#fff", padding: "6px 10px", border: "1px solid rgba(255,255,255,0.2)", textAlign: "left" }}>Metric</th>
                  {hours.map((h, i) => (
                    <th key={i} style={{ background: accent, color: "#fff", padding: "6px 8px", border: "1px solid rgba(255,255,255,0.2)", textAlign: "center", minWidth: 52 }}>H{h.HourNo}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {[
                  ...
                  {
                    label: "Achiev. %", isObj: true, vals: hours.map(h => {
                  ...
                  <tr key={ri} style={{ background: ri % 2 === 0 ? "#fff" : "#f8fafc" }}>
                    <td style={{ padding: "5px 10px", border: "1px solid #f1f5f9", color: "#334155", fontWeight: 600 }}>{row.label}</td>
                    {hours.map((_, ci) => {
                      const item = row.isObj ? row.vals[ci] : { v: row.vals[ci], c: row.color };
                      return (
                        <td key={ci} style={{ padding: "5px 8px", border: "1px solid #f1f5f9", textAlign: "center", fontWeight: 700, color: item.c }}>{item.v}</td>
                      );
                    })}
                  </tr>
                ))}
          ...
          <div style={{
            flex: 1, background: "#fff", border: "1px solid #f1f5f9", borderRadius: 10,
            padding: "8px 8px 4px", minHeight: 120, display: "flex", flexDirection: "column",
          }}>
            <div style={{ display: "flex", gap: 14, justifyContent: "center", marginBottom: 6, fontSize: 11 }}>
              {[["#1e40af","Target"],["#d97706","Actual"],["#ef4444","Loss"]].map(([cl, l]) => (
                <span key={l} style={{ display: "flex", alignItems: "center", gap: 4, color: "#64748b" }}>
                  <span style={{ width: 10, height: 10, background: cl, borderRadius: 2, display: "inline-block" }}/>{l}
                </span>
              ))}
            </div>
...
const Page4 = ({ apiData = {}, progress, shift, shiftDate }) => {
  ...
  const pieData = useMemo(() => ({
    ...
    datasets: [{
      ...
      borderColor: ["#15803d", "#b91c1c"], borderWidth: 2, hoverOffset: 0,
    }],
  }), [qs]);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", width: "100%", height: "100%", background: "#f8fafc" }}>
      <PageHeader title="Quality Performance" shift={shift} shiftDate={shiftDate} accent={accent}/>
      <TimerBar progress={progress} accent="#22c55e"/>
      <div style={{ display: "flex", flex: 1, minHeight: 0 }}>
        <div style={{
          width: 190, flexShrink: 0, display: "flex", flexDirection: "column",
          gap: 10, padding: "14px 12px", background: "#f8fafc",
          borderRight: "1px solid #f1f5f9", justifyContent: "center",
        }}>
          <div style={{ position: "relative", display: "flex", justifyContent: "center" }}>
          ...
          <div style={{ background: "#fff", border: "1px solid #f1f5f9", borderRadius: 10, overflow: "hidden" }}>
            {[
              ...
              <div key={i} style={{
                ...
                fontSize: 11, color: "#94a3b8", borderBottom: "1px solid #f8fafc",
              }}>
                <span>{k}</span>
        ...
        <div style={{
          ...
          padding: "12px 20px", gap: 20, minWidth: 0,
        }}>
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
            <div style={{ fontSize: 13, color: "#0f172a", fontWeight: 700 }}>Quality Overview</div>
            <div style={{ position: "relative" }}>
              ...
              <div style={{
                ...
                <div style={{ fontSize: 26, fontWeight: 800, color: "#0f172a" }}>{qs.TotalAchieved ?? 0}</div>
                <div style={{ fontSize: 10, color: "#94a3b8" }}>Total</div>
              </div>
            </div>
            <div style={{ display: "flex", gap: 14, fontSize: 11 }}>
              <span style={{ display: "flex", alignItems: "center", gap: 4, color: "#15803d" }}>
                <span style={{ width: 10, height: 10, background: "#15803d", borderRadius: 2, display: "inline-block" }}/> OK: {qs.OkUnit ?? 0}
              </span>
              <span style={{ display: "flex", alignItems: "center", gap: 4, color: "#ef4444" }}>
                <span style={{ width: 10, height: 10, background: "#ef4444", borderRadius: 2, display: "inline-block" }}/> Defect: {qs.DefectUnit ?? 0}
              </span>
            </div>
          </div>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{
              background: accent, color: "#fff", fontWeight: 700, fontSize: 13,
              textAlign: "center", padding: 8, borderRadius: "8px 8px 0 0",
            }}>Top Defects Today</div>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
              <thead>
                <tr>
                  {["Sr.", "Defect Description", "Count"].map(h => (
                    <th key={h} style={{
                      background: "#dcfce7", color: accent, padding: "7px 10px",
                      border: "1px solid #f1f5f9", textAlign: "left", fontWeight: 700,
                    }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {defects.length > 0 ? defects.map((df, i) => (
                  <tr key={i} style={{ background: i % 2 === 0 ? "#fff" : "#f8fafc" }}>
                    <td style={{ padding: "7px 10px", border: "1px solid #f1f5f9", textAlign: "center", color: "#94a3b8" }}>{df.SrNo}</td>
                    <td style={{ padding: "7px 10px", border: "1px solid #f1f5f9", color: "#334155" }}>{df.DefectName}</td>
                    <td style={{ padding: "7px 10px", border: "1px solid #f1f5f9", textAlign: "center", fontWeight: 800, color: "#ef4444" }}>{df.DefectCount}</td>
                  </tr>
                )) : (
                  <tr>
                    <td colSpan={3} style={{ padding: 24, textAlign: "center", color: "#94a3b8", border: "1px solid #f1f5f9" }}>
...
const Page5 = ({ apiData = {}, progress, shift, shiftDate }) => {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", width: "100%", height: "100%", background: "#f8fafc" }}>
      <PageHeader title="Loss Performance" shift={shift} shiftDate={shiftDate} accent={accent}/>
      <TimerBar progress={progress} accent="#f59e0b"/>
      <div style={{ display: "flex", flex: 1, minHeight: 0 }}>
        <SidebarPanel
          pct={ls.ConsumedTimePct} fillColor="#f59e0b"
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
            ...
            <tbody>
              {stations.map((s, i) => {
                ...
                const rowBg  = isCritical ? `rgba(239,68,68,${0.08 + intensity * 0.12})` : isHigh ? "#fef3c7" : "#fff";
                const textCol = isCritical ? "#ef4444" : isHigh ? "#b45309" : "#334155";
                return (
                  <tr key={i} style={{ background: rowBg }}>
                    <td style={{ padding: "8px 12px", border: "1px solid #f1f5f9", color: "#334155", fontWeight: 600 }}>{s.StationName}</td>
                    <td style={{ padding: "8px 12px", border: "1px solid #f1f5f9", color: textCol, textAlign: "center", fontWeight: 700 }}>{s.TotalStopTimeHMS ?? "—"}</td>
                    <td style={{ padding: "8px 12px", border: "1px solid #f1f5f9", color: textCol, textAlign: "center", fontWeight: 700 }}>{s.TotalStopTime}</td>
                    <td style={{ padding: "8px 12px", border: "1px solid #f1f5f9", color: s.TotalStopCount > 10 ? "#ef4444" : "#334155", textAlign: "center", fontWeight: 700 }}>{s.TotalStopCount}</td>
                  </tr>
                );
              })}
              {stations.length === 0 && (
                <tr>
                  <td colSpan={4} style={{ padding: 28, textAlign: "center", color: "#94a3b8", border: "1px solid #f1f5f9" }}>
  ...
  if (isLaunched) {
    return (
      <div style={{
        ...
        flexDirection: "column", background: "#f8fafc", overflow: "hidden",
        ...
        <div style={{
          ...
          {launchedConfig && (
            <span style={{ fontWeight: 800, fontSize: 13, color: "#0f172a", display: "flex", alignItems: "center", gap: 7 }}>
          ...
          {["A", "B"].map(s => (
            <button key={s} onClick={() => {
              ...
              const dt = s === "B"
                ...
                : todayISO();
              setShift(s); setShiftDate(dt);
              fetchData(dt, s, launchedConfig);
            }} style={{
              padding: "4px 12px", borderRadius: 7, fontSize: 12, fontWeight: 700, cursor: "pointer",
              background: shift === s ? (s === "A" ? "#dbeafe" : "#fef3c7") : "#f8fafc",
              color:      shift === s ? (s === "A" ? "#1e40af" : "#d97706") : "#94a3b8",
          ...
          <input type="date" value={shiftDate} onChange={e => setShiftDate(e.target.value)} style={{
            padding: "4px 10px", border: "1.5px solid #f1f5f9", borderRadius: 7,
          ...
          <div style={{ marginLeft: "auto", display: "flex", gap: 8, alignItems: "center" }}>
            ...
            {isRunning ? (
              <button onClick={() => setIsRunning(false)} style={{
                ...
                background: "#fef3c7", color: "#d97706", border: "1.5px solid #fcd34d",
              }}>
        ...
        {loading && (
          <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: "#fff" }}>
            <FiRefreshCw size={32} color="#6366f1" style={{ animation: "spin 1s linear infinite" }}/>
            <div style={{ color: "#94a3b8", marginTop: 14, fontSize: 14 }}>Fetching shift data…</div>
          </div>
        )}
        ...
        {!loading && lastFetched && (
          <div style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column" }}>
            ...
            <NavDots currentPage={currentPage} onGoTo={goTo} onPrev={goPrev} onNext={goNext}/>
          </div>
        )}
        ...
        {!loading && !lastFetched && (
          <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: "#fff" }}>
            <div style={{ width: 72, height: 72, borderRadius: 18, background: "#f1f5f9", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 16 }}>
              <FiSettings size={32} color="#cbd5e1"/>
            </div>
            <div style={{ fontSize: 16, fontWeight: 700, color: "#475569" }}>Loading dashboard data…</div>
          </div>
        )}
    ...
    <div style={{ padding: 24, background: "#f8fafc", minHeight: "100vh", fontFamily: "system-ui, -apple-system, sans-serif" }}>
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          <div style={{
            ...
            <span style={{ color: "#fff", fontWeight: 900, fontSize: 20 }}>W</span>
          </div>
          <div>
            <h1 style={{ margin: 0, color: "#0f172a", fontSize: 20, fontWeight: 800, fontFamily: "'Georgia', serif" }}>
              Final Area Dashboard
            </h1>
        ...
        {lastFetched && (
          <div style={{ color: "#94a3b8", fontSize: 12, textAlign: "right" }}>
            ...
            <div style={{ fontWeight: 700, color: "#0f172a" }}>{lastFetched.toLocaleTimeString()}</div>
          </div>
        )}
      </div>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 16, alignItems: "flex-end" }}>
          <div>
            <div style={{ fontSize: 11, fontWeight: 700, color: "#94a3b8", marginBottom: 5, textTransform: "uppercase", letterSpacing: 0.5 }}>Shift Date</div>
            <input type="date" value={shiftDate} onChange={e => setShiftDate(e.target.value)} style={{
              padding: "9px 12px", border: "1.5px solid #e2e8f0", borderRadius: 9,
              fontSize: 13, outline: "none", color: "#0f172a", background: "#f8fafc",
            }}/>
          </div>
          <div>
            <div style={{ fontSize: 11, fontWeight: 700, color: "#94a3b8", marginBottom: 5, textTransform: "uppercase", letterSpacing: 0.5 }}>Shift</div>
            <div style={{ display: "flex", background: "#f8fafc", border: "1.5px solid #e2e8f0", borderRadius: 9, overflow: "hidden" }}>
              {[["A","08:00–20:00"],["B","20:00–08:00"]].map(([s]) => (
                <button key={s} onClick={() => setShift(s)} style={{
                  padding: "9px 18px", fontSize: 13, fontWeight: 700, cursor: "pointer",
                  background: shift === s ? "#1e40af" : "transparent",
                  color: shift === s ? "#fff" : "#94a3b8",
                  border: "none", borderRight: "1.5px solid #e2e8f0", transition: "all 0.15s",
                }}>Shift {s}</button>
              ))}
            </div>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={fetchAllData} disabled={loading} style={{
              ...
              background: loading ? "#94a3b8" : "#1e40af", color: "#fff", border: "none",
              borderRadius: 9, fontWeight: 700, fontSize: 13, cursor: loading ? "not-allowed" : "pointer",
      ...
      {loading && (
        <div style={{
          ...
          padding: 64, background: "#fff", borderRadius: 12, border: "1px solid #f1f5f9",
        }}>
          <FiRefreshCw size={32} color="#1e40af" style={{ animation: "spin 1s linear infinite" }}/>
          <div style={{ color: "#94a3b8", marginTop: 14, fontSize: 14 }}>Fetching shift data…</div>
        </div>
      )}

      {!loading && lastFetched && (
        <div style={{ borderRadius: 14, overflow: "hidden", border: "1px solid #f1f5f9", boxShadow: "0 2px 16px rgba(0,0,0,0.07)" }}>
          <div style={{ height: "70vh", overflow: "hidden" }}>{pages[currentPage]}</div>
          <NavDots currentPage={currentPage} onGoTo={goTo} onPrev={goPrev} onNext={goNext}/>
        </div>
      )}
      ...
      {!loading && !lastFetched && (
        <div style={{
          ...
          <div style={{ fontSize: 16, fontWeight: 700, color: "#475569", marginBottom: 6 }}>No data loaded</div>
          <div style={{ fontSize: 13, color: "#94a3b8" }}>Select a date and click Load Dashboard to begin</div>
        </div>
      )}
