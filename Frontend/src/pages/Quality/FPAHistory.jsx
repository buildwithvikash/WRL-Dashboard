...
const exportToCSV = (data, label = "fpa_report") => {
  ...
  // BUG FIX #5: Column order now matches the table display order
  const headers = [
    "Model Name",
    "Production",
    "FPA Req.",
...
const ChartTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg shadow-xl p-3 text-xs min-w-[150px]">
      <p className="font-bold text-gray-700 mb-2 truncate max-w-[200px]">
        {label}
      </p>
      {payload.map((p) => (
        <div
...
/* Inline Sort Icon */
// BUG FIX #4: SortIcon alignment now follows the column, not forced to center
const SortIcon = ({ field, sortField, sortDir }) => {
  if (sortField !== field)
    return <FiMinus size={9} className="opacity-25 ml-0.5" />;
  return sortDir === "asc" ? (
...
const ModelAlertCard = ({ model, index, onView }) => {
  ...
  return (
    <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg border border-gray-100 hover:border-orange-200 hover:bg-orange-50/30 transition-all">
      ...
      <span
        className={`text-[10px] font-bold px-2 py-0.5 rounded border ${st?.bg} ${st?.text} ${st?.border} flex-shrink-0`}
      >
        {fpqi.toFixed(2)}
      </span>
    </div>
...
const FpaHistory = () => {
  ...
  /*
   * BUG FIX #2: FPQI ranking — "worst first" means highest FPQI at top.
   * Previously `.reverse()` was called after slicing which flipped the order
   * to best-first, contradicting the panel title "Worst First".
   * FIX: Sort descending (worst first) and do NOT reverse, so the horizontal
   * bar chart renders worst model at the top (first entry = top Y-axis item).
   */
  ...
  // BUG FIX #6: Memoize the FPQI watchlist data instead of computing inline twice
  ...
  /*
   * BUG FIX #1 — Critical/Major/Minor not showing in detail modal.
   *
   * Root cause: `openModelModal(item.ModelName)` passes only the model name
   * string. The `FpaModelDetailModal` has no way to access defect counts
   * (Critical, Major, Minor) unless it re-fetches them separately.
   *
   * If your Redux slice + modal are designed to re-fetch data by model name,
   * the defect counts must come from that API response — make sure the API
   * returns Critical/Major/Minor fields and the modal reads them.
   *
   * If the modal is NOT doing a separate fetch (just reading from Redux store),
   * you MUST dispatch the full row object instead of just the model name.
   * Two possible fixes depending on your slice design:
   *
   * Option A — slice stores the full selected model object:
   *   dispatch(openModelModal(item))  // pass whole row object
   *
   * Option B — slice stores only name (current), but modal fetches by name:
   *   Ensure the API endpoint called by the modal returns Critical/Major/Minor.
   *   No change needed here; fix is in FpaModelDetailModal or the API.
   *
   * We implement Option A below (safest, works without API changes).
   * Update your fpaReportSlice `openModelModal` reducer to accept an object:
   *   openModelModal: (state, action) => {
   *     state.isModelModalOpen = true;
   *     state.selectedModel = action.payload; // full object, not just string
   *   }
   * And read selectedModel.ModelName wherever you need the name in the modal.
   */
  const handleOpenModelModal = (item) => {
    dispatch(openModelModal(item)); // FIX: pass full item object, not just ModelName
  };
  ...
  /*
   * BUG FIX #3 — Pagination page number generation.
   * Original formula: `Math.max(1, Math.min(totalPages - 4, page - 2)) + i`
   * could produce page numbers > totalPages when totalPages < 5.
   * Fixed with a proper window calculation that clamps to valid range.
   */
        ...
        <div className="max-w-screen-2xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-blue-300/70 text-[11px] mt-0.5 font-medium">
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-3 items-end">
            ...
            <div className="md:col-span-3 flex items-end gap-2">
              <Button
                onClick={handleQuery}
                bgColor={
                  isLoading ? "bg-gray-300" : "bg-blue-700 hover:bg-blue-800"
                }
              ...
              <Button
                onClick={handleReset}
                bgColor="bg-gray-100 hover:bg-red-50"
                textColor="text-gray-500 hover:text-red-600"
            ...
            <div className="md:col-span-3 flex items-end gap-2">
              <span className="text-[10px] text-gray-400 font-semibold whitespace-nowrap mb-[10px]">
                Quick:
              </span>
              {[
          ...
          <>
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-3 flex items-center gap-3">
                <span className="bg-blue-50 p-2 rounded-lg">
                  <MdOutlineFactory size={18} className="text-blue-600" />
                </span>
                <div>
              ...
              <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-3 flex items-center gap-3">
                <span className="bg-teal-50 p-2 rounded-lg">
                  <HiOutlineBadgeCheck size={18} className="text-teal-600" />
                </span>
                <div>
                  <p className="text-[10px] text-gray-400 font-semibold uppercase tracking-wider">
                    FPA Required
                  ...
                  <p className="text-lg font-black text-gray-800">
                    {kpis.totalFPAReq.toLocaleString()}
                  </p>
                </div>
              </div>
              <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-3 flex items-center gap-3">
            ...
            <div>
              ...
              >
                ...
                Charts & Analytics
                <span
                  className={`ml-1 transition-transform ${chartsExpanded ? "rotate-180" : ""}`}
                >
                  ?
                </span>
              </button>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-4 gap-4 items-start">
                  ...
                  >
                    <div className="p-3 space-y-2">
                      <p className="text-[10px] text-gray-400 mb-2">
                        Models with FPQI &gt; 2.000 — needs attention
                      </p>
                      {worstModels.map((m, i) => (
              ...
              <div className="xl:col-span-3">
                ...
                >
                      ...
                      <tbody className="divide-y divide-gray-50">
                        {pagedData.map((item, idx) => {
                          ...
                          return (
                            ...
                            >
                              <td className="px-3 py-2.5 text-gray-300 text-[10px]">
                                {globalIdx}
                              </td>
                              ...
                              <td className="px-3 py-2.5 max-w-[220px]">
                                <span
                                  className="font-semibold text-gray-800 truncate block text-[11px]"
                              ...
                              {/* Defects — BUG FIX: guard against undefined with || 0 */}
                              <td className="px-3 py-2.5 text-center">
                                <span
                                  className={`font-black text-sm ${(item.Critical || 0) > 0 ? "text-red-600" : "text-gray-200"}`}
                                >
                                  {item.Critical || 0}
                                </span>
                              </td>
                              <td className="px-3 py-2.5 text-center">
                                <span
                                  className={`font-black text-sm ${(item.Major || 0) > 0 ? "text-orange-500" : "text-gray-200"}`}
                                >
                                  {item.Major || 0}
                                </span>
                              </td>
                              <td className="px-3 py-2.5 text-center">
                                <span
                                  className={`font-black text-sm ${(item.Minor || 0) > 0 ? "text-yellow-500" : "text-gray-200"}`}
                                >
                                  {item.Minor || 0}
                                </span>
                              </td>
                              ...
                              <td className="px-3 py-2.5 text-center">
                                {fpqi !== null ? (
                                  <span
                                    className={`inline-block px-1.5 py-0.5 rounded border text-[10px] font-black ${st?.bg} ${st?.text} ${st?.border}`}
                                  >
                                    {fpqi.toFixed(3)}
                                  </span>
                                ) : (
                              ...
                              {/* Action — BUG FIX #1: pass full item object */}
                              <td className="px-3 py-2.5 text-center">
                                {hasInsp ? (
                                  <button
                  ...
                  {/* Table Footer + Pagination — BUG FIX #3: correct page numbers */}
                  <div className="px-4 py-3 bg-gray-50 border-t border-gray-100 flex flex-wrap items-center justify-between gap-3 text-[11px] text-gray-500">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      {/* FIX #3: use memoized pageNumbers array */}
                      {pageNumbers.map((p) => (
                        <button
                          key={p}
                          onClick={() => setPage(p)}
                          className={`w-7 h-7 rounded border text-[10px] font-bold transition-all ${
                            p === page
                              ...
                              : "bg-white border-gray-200 hover:bg-blue-50 text-gray-600"
                          }`}
                        >
                          {p}
                        </button>
            ...
            {/* BUG FIX #6: use memoized watchlistData */}
            <Panel
                      ...
                      {watchlistData.map((r, i) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="px-4 py-2.5 font-semibold text-gray-800 max-w-[280px] truncate">
                              {r.ModelName}
                            </td>
                            ...
                            {/* BUG FIX #7: guard defect values with || 0 */}
                            <td className="px-4 py-2.5 text-center text-red-600 font-black">
                              {r.Critical || 0}
                            </td>
                            <td className="px-4 py-2.5 text-center text-orange-600 font-black">
                              {r.Major || 0}
                            </td>
                            <td className="px-4 py-2.5 text-center text-yellow-600 font-black">
                              {r.Minor || 0}
                            </td>
                            ...
                            <td className="px-4 py-2.5 text-center">
                              <span
                                className={`px-2 py-0.5 rounded border text-[10px] font-black ${st?.bg} ${st?.text} ${st?.border}`}
                              >
                                {st?.label}
                              </span>
                            </td>
                            {/* BUG FIX #1 + #7: consistent hasInsp guard + pass full object */}
                            <td className="px-4 py-2.5 text-center">
                              {r.SampleInspected > 0 ? (
                                <button
