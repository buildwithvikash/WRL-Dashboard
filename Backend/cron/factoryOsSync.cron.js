...
export const startFactoryOsSyncCron = () => {
  // Refresh the in-progress production day every five minutes.
  cron.schedule("*/5 * * * *", syncToday, { timezone: "Asia/Kolkata" });
  console.log("[FactoryOS Sync] Cron started — syncing every 5 minutes.");
  setTimeout(syncToday, 10_000); // Allow DB migrations to create the log table first.
};
